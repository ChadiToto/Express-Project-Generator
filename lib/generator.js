...
/**
 * This function add Imports in case of an embedded field
 * @param {*} data
 * @param {*} model
...
function setImport(data, model) {
  ...
  for (let field of model.fields) {
    ...
    if (!primative_types.includes(type)) {
      let _import = "var { " + type + "Schema } = require('./" + type + ".js')"; // String to be added
      let re = new RegExp(field.type, "g"); // Regexp to be replaced
...
function generateModel(model) {
  ...
  fs.readFile(model_file, "utf8", function (err, data) {
    ...
    generated_values = setImport(generated_values, model);
