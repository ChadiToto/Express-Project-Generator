...
function generateModel(model) {
  ...
  fs.readFile(model_file, "utf8", function (err, data) {
    ...
    generated_values = generated_values.replace("fields", setFields(model)); // Insert Fields
...
function generateController(model) {
  ...
  fs.readFile(controller_file, "utf8", function (err, data) {
    ...
    /* Modify Template Values */
    var generated_values = data.replace(/Model/g, capitalize(model.name)); // Replace Variables
    generated_values = generated_values.replace(
      /mymodel/g,
      model.name.toLowerCase()
    ); // get Importfile
    generated_values = generated_values.replace("fields", getBody(model));

    /* Apply Changes */
    fs.writeFileSync(controller_file, generated_values, "utf8");

    console.log(("\nCreated Controller : " + model.name).green);
  });
}
